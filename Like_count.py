...
class ArabicDataset(data.Dataset):
    ...
    def __init__(self, data_df, tweet, label_onehot, label, like_count, quote_count, reply_count, retweet_count,
        ...
        self.metadata_number = torch.tensor(like_count, dtype=torch.float).unsqueeze(1)
...
class CNNBiLSTM(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout):
        ...
        self.embedding = nn.Linear(input_dim, embedding_dim)  #（20，128）-->(1,64)
        self.conv = nn.Conv1d(in_channels=embedding_dim, out_channels=16, kernel_size=1)  #（128，32，1）-->(64,16,1)
        self.rnn = nn.LSTM(16,            #（32，64，1）-->(16,32,1)
                  hidden_dim,
                  num_layers=n_layers,
                  bidirectional=False,
                  dropout=dropout)
        self.fc = nn.Linear(hidden_dim, output_dim)     #(64*2=128,3)-->(32*2=64,3)
        self.dropout = nn.Dropout(dropout)
    def forward(self, metadata):
        #metadata = [batch size, metadata dim] = [32,128]-->[32,1]

        embedded = self.dropout(self.embedding(metadata))
        #embedded = [batch size, metadata dim, emb dim]=[32,1,64]

        embedded = torch.reshape(embedded, (metadata.size(0), 64, 1))  #[32,64,1]

        conved = F.relu(self.conv(embedded))  # activation function:ReLu
        #conved = [batch size, n_filters, metadata dim - filter_sizes[n] + 1]

        conved = torch.reshape(conved, (metadata.size(0), 16))
