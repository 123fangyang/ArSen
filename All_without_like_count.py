...
class ArabicDataset(data.Dataset):
    ...
    def __init__(self, data_df, tweet, label_onehot, label, quote_count, reply_count, retweet_count,
        ...
        self.metadata_number = torch.cat((torch.tensor(quote_count, dtype=torch.float).unsqueeze(1),
...
test_dataset = ArabicDataset(test_data, test_text, test_label, torch.tensor(test_data['label'].replace(label_convert)),
                              test_data['quote_count'].tolist(), # 计数
...
input_dim_metadata = 8        #8个数值型的feature, all_without_like_count
hidden_dim = 64
n_layers = 3
bidirectional = True
...
def test(model, test_loader, model_save):
    ...
    test_f1_ave = f1_score(test_label_all, test_predict_all, average='weighted')

    print(f'Test F1 Average: {test_f1_ave:.4f}')
