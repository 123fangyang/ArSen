...
model_save = 'Interacting metric.pt'
model_name = 'Interacting metric'
num_epochs = 20
batch_size = 32
learning_rate = 1e-3
num_classes = 3
...
class ArabicDataset(data.Dataset):
    def __init__(self, data_df, tweet, label_onehot, label, like_count, quote_count, reply_count, retweet_count,
        ...
        self.label = label
        self.metadata_number = torch.cat((torch.tensor(like_count, dtype=torch.float).unsqueeze(1), torch.tensor(quote_count, dtype=torch.float).unsqueeze(1),
                                   torch.tensor(reply_count, dtype=torch.float).unsqueeze(1), torch.tensor(retweet_count, dtype=torch.float).unsqueeze(1)), dim=-1)
