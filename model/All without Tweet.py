...
model_save = 'All without Tweet.pt'
model_name = 'All without Tweet'
num_epochs = 20
batch_size = 32
learning_rate = 1e-3
num_classes = 3
...
class ArabicDataset(data.Dataset):
    def __init__(self, data_df,label_onehot, label , like_count, quote_count, reply_count, retweet_count,
...
def train(num_epochs, model, train_loader, val_loader, optimizer, criterion, model_save):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = 0.0
        train_accuracy = 0.0
        for label_onehot, label,metadata_number in train_loader:
            label_onehot = label_onehot.to(DEVICE)
            label = label.to(DEVICE)
            metadata_number = metadata_number.to(DEVICE)

            optimizer.zero_grad()
            outputs = model(metadata_number)
...
# Validation
        ...
        val_accuracy = 0.0
        with torch.no_grad():
            for label_onehot, label,metadata_number in val_loader:
                label_onehot = label_onehot.to(DEVICE)
                label = label.to(DEVICE)
                metadata_number = metadata_number.to(DEVICE)

                val_outputs = model(metadata_number)
...
def test(model, test_loader, model_save):
    ...
    test_accuracy = 0.0
    with torch.no_grad():
        for label_onehot, label,metadata_number in test_loader:
            label_onehot = label_onehot.to(DEVICE)
            label = label.to(DEVICE)
            metadata_number = metadata_number.to(DEVICE)

            test_outputs = model(metadata_number)
