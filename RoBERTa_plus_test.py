...
def train(epoch):
    ...
    with torch.no_grad():
        for step, data in enumerate(testing_loader, 0):
            ...
            test_loss += loss.item()
            _, predicted = torch.max(outputs.data, dim=1)
            test_correct += calcuate_accuracy(predicted, targets)
            nb_test_steps += 1
    ...
    test_loss_list.append(test_loss / nb_test_steps)
    test_acc_list.append((test_correct) / test_total)
    test_f1_list.append(test_f1_weighted)

    # Calculate and log test loss, accuracy, and F1 score
    ...
    print(f"[Epoch {epoch}] Test F1 (Weighted) Epoch: {test_f1_weighted:.4f}")
    print(f"[Epoch {epoch}] Test F1 (Macro) Epoch: {test_f1_macro:.4f}")
    print(f"[Epoch {epoch}] Test F1 (Micro) Epoch: {test_f1_micro:.4f}\n")
